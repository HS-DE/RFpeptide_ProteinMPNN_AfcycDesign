    ...
    if binderlen > 0:
        if ppi_conf.hotspot_res is not None:
            ...
            for i, res in enumerate(mappings["receptor_con_ref_pdb_idx"]):
                if res in hotspots:
                    # receptor_con_hal_idx0 is receptor-local. Model tensors are
                    # complex-global with the generated binder placed first.
                    hotspot_idx.append(mappings["receptor_con_hal_idx0"][i] + binderlen)
    return hotspot_idx
