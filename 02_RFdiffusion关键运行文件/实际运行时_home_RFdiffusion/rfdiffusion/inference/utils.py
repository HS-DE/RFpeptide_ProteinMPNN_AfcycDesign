    ...
    if binderlen > 0:
        if ppi_conf.hotspot_res is not None:
            ...
            for i, res in enumerate(mappings["receptor_con_ref_pdb_idx"]):
                if res in hotspots:
                    # receptor_con_hal_idx0 is receptor-local in this RFpeptides
                    # fork. The model tensor contains the binder first, so hotspot
                    # features require complex-global indices.
                    hotspot_idx.append(
                        mappings["receptor_con_hal_idx0"][i] + binderlen
                    )
    return hotspot_idx
