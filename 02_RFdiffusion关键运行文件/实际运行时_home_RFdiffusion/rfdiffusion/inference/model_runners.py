...
class Sampler:
    ...
    def sample_init(self, return_forward_trajectory=False):
        ...
        if requested_hotspots and len(self.hotspot_0idx or []) != len(requested_hotspots):
            raise RuntimeError(
                "Not all requested target hotspots were mapped into the RFpeptides complex: "
                f"requested={requested_hotspots}, mapped={self.hotspot_0idx}"
            )
        if any(index < self.binderlen for index in (self.hotspot_0idx or [])):
            raise RuntimeError(
                "RFpeptides hotspot mapping remained receptor-local instead of complex-global: "
                f"binderlen={self.binderlen}, hotspot_0idx={self.hotspot_0idx}"
            )
        self._log.info(
            "Mapped target hotspots to complex-global indices: binderlen=%s hotspot_0idx=%s",
            self.binderlen,
            self.hotspot_0idx,
        )
