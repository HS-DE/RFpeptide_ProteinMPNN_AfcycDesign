...
| `CURRENT_CORRECTED_SMOKE_REFERENCE` | 修复后 N20 实际 runner 样例 |
...
02_RFdiffusion关键运行文件/ 直接参与 Stage 1 hotspot mapping 的 5 个文件
03_旧错误路线生成脚本/     归档中实际存在的 100 个 job/runner，禁止执行
04_修复后运行脚本样例/     修复后的 N20 Stage 1 runner
...
| 文件 | 用途和检查重点 |
| --- | --- |
| `scripts/run_inference.py` | Hydra inference 入口；确认传入 contig、cyclic chain 和 hotspot 参数 |
...
3. `rfdiffusion/inference/utils.py`：确认 hotspot index 加 `binderlen`。
4. `rfdiffusion/inference/model_runners.py`：确认模型 feature 使用同一修正 index。
5. `20_make_rfpeptides_article_jobs.py`：确认 runner preflight 会在 offset 错误时终止。
6. `04_修复后运行脚本样例`：核对实际 contig、target PDB 和 hotspot 参数。
7. `21_collect_rfpeptides_backbones.py`：确认 crop position 按 target-chain order 映射，且 near-only 默认失败。
8. `23_collect_proteinmpnn_sequences.py`：确认 Stage 3 继续使用每个结构自己的 mapped residue number。
9. `25_stage4_rosetta_interface_scoring.py`：确认不移动 backbone，分数不被描述为实验结合能。
10. `26-31`：确认 Stage 5 identity/cache、template coverage、坐标对齐和 reference-site premise。

## 当前 smoke 状态

修复后的 N20 已生成并用严格 Stage 2 门槛重跑：

```text
20 parsed
3 strict pass: RFpep_Site_2_0002, RFpep_Site_2_0017, RFpep_Site_2_0018
all 3 have direct hotspot contacts
all 3 have no severe clash
all 3 pass head-to-tail terminal geometry
```

这三条仍需逐条 PyMOL 人工检查，不能据此直接扩大生产。
