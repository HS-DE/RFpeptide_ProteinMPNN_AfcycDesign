...
def _load_runtime_audit(pdb_path: Path, trb_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
...
def _qc_row_for_design(
    *,
    ...
    try:
        trb_data, runtime_audit = _load_runtime_audit(pdb_path, trb_path)
    except Exception as exc:
        runtime_failures.append(f"runtime_audit_unreadable:{exc.__class__.__name__}")
    ...
    if not runtime_audit:
        ...
        expected_hotspots = [
            f"{target_chain_expected}{number}"
            for number in sorted(hotspot_residue_numbers, key=_residue_sort_key)
        ]
        if sorted(str(value) for value in configured_hotspots or []) != sorted(expected_hotspots):
    ...
    expected_hotspot_0idx = sorted(
        len(peptide_residues) + _parse_int(number) - 1
        for number in hotspot_residue_numbers
    )
    ...
    if not require_runtime_audit and runtime_failures:
        runtime_notes.append("Runtime audit failures were retained as warnings by --allow-missing-runtime-audit.")
    ...
    base_row.update(
        {
            ...
            "runtime_chain_idx_status": (
                ...
                if runtime_audit
                and [str(value) for value in runtime_audit.get("chain_idx", [])] == _pdb_residue_chain_order(chains)
...
Production QC also requires the Stage 1 TRB `runtime_audit`: source commit and
hashes must match the Stage 1 job table, hotspot indices must equal
`peptide_length + crop_position - 1`, cyclic indices must cover peptide only,
and the target chain must exactly match the Stage 0 target sequence. This gate
prevents a preflight from checking one RFdiffusion checkout while inference
loads another.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Legacy inspection only. By default, missing or mismatched Stage 1 runtime provenance fails Stage 2.",
    )
